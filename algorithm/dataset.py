...
class Dataset(object):
    ...
    def get_training_images(self, dataset_path, args):
        # if settings of datasets and paths are the same, the have same hash 
        ...
        if os.path.exists(current_file_name):
            ...
            self.render_training_images() # !!!!!!!!!!! image generation !!!!!!!!!!!!!!!
            np.savez(current_file_name, train_x = self.train_x, mask_x = self.mask_x, train_y = self.train_y)
        self.noof_obj_pixels = np.count_nonzero(self.mask_x==0,axis=(1,2)) # num of pixel inside object in each picture
    ...
    def load_bg_images(self, dataset_path):
        # hash according to shape, noof_bg_images and background images path
    ...
    '''
    '''
    # diff btw x and y : lighting and cropping
    ...
    def _aug_occl(self):
        ...
        return Sometimes(0.5, CoarseDropout( p=0.2, size_percent=0.05) )
    '''
    @lazy_property
    # realistic occlusion masks from simulation
    ...
    def augment_squares(self,masks,rand_idcs,max_occl=0.25):
        ...
        while len(idcs) > 0: # choosing occlusion until all occlu < 0.25
    ...
    def batch_grasp(self, batch_size):
        ...
        for idx in rand_idcs:
            img = cv2.imread(os.path.join(data_path,'{}.png'.format(idx)))
            batch_x.append(img)
            if eval(self._kw['blur']):
                img = cv2.bilateralFilter(img.copy(),11,150,150)
            batch_y.append(img)
        ...
        # rand_vocs = self.bg_imgs[rand_idcs_bg]
        white = np.ones(batch_y.shape,np.uint8) * 255
        masks = white
        # occluded mask

        # should generate realistic occlusion mask data first
        if eval(self._kw['realistic_occlusion']):
            masks = self.augment_occlusion_mask(masks.copy(),max_occl=np.float(self._kw['realistic_occlusion']))
        # from imgaug package
        if eval(self._kw['square_occlusion']):
            # masks = self.augment_squares(white.copy(),rand_idcs,max_occl=np.float(self._kw['square_occlusion']))
            masks = self._aug_occl.augment_images(white)
            masks = masks.astype(np.bool)
            masks = np.invert(masks,dtype=np.bool)
    ...
    def batch_embedding_grasp(self, start, end):

        
        data_path = self.dataset_path
        idcs = list(range(start,end))
