...
def rectangle(center,angle,w,h):
	...
	rotated_box = np.transpose(np.dot(R,np.transpose(box)))
	trans = np.array([center[0],center[1]]*4).reshape(4,2)
	final_box = rotated_box + trans
	
	return final_box.astype(np.int)
